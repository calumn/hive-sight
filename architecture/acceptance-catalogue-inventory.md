...
| varroa | One-click Varroa Photo Analysis workflow | `apps/web/tests/acceptance/slice-0008-inspection-intent-and-multi-photo-intake.spec.ts` | Web UI | web-only | keep as browser-specific coverage | Slice 0034 browser evidence for Varroa Assessment photo upload, `Analyze photo`, evidence display, review controls, resume after Development User switching, and absence of Training workflow controls. |
