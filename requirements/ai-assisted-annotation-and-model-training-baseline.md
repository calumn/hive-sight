...
**Draft Annotation**:
A proposed Annotation generated by a model, hosted vision service, generative AI tool, or other pre-labelling mechanism before human review.
...
**Annotation Source**:
The provenance of an Annotation or Draft Annotation, such as `human_from_scratch`, `ai_assisted_draft`, `model_prediction`, `user_correction`, or `reviewer_correction`.
...
The system should support creating Draft Annotations for original inspection photos using an AI-assisted pre-labelling mechanism.
...
Draft Annotations shall require human review, correction, or rejection before they become Reviewed Annotations.
...
Every Draft Annotation, Reviewed Annotation, Dataset Item, Dataset Version, Training Run, Model Candidate, and Benchmark Evaluation shall preserve provenance sufficient to explain where it came from and how it was used.
...
- may use AI-assisted Draft Annotations
...
- The first implementation may use a deterministic or manual stub pre-labeller, with a replaceable pre-labelling seam.
...
- HiveSight's canonical annotation storage starts as internal normalized JSON. YOLO, COCO, or other export formats are deferred until model/tool choice.
- Dataset Role assignment is a separate curation step after annotation review.
- Reviewers should visibly know when annotations are AI-assisted draft suggestions.
- Benchmark items may originate from AI-assisted Draft Annotations if human reviewed and provenance is preserved.
...
- Which pre-labelling mechanism should HiveSight use first: hosted vision model, hosted annotation platform, open-source detector, generative AI vision model, or manual external tool?
...
- What exact annotation export format should be used first?
