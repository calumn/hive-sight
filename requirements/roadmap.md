...
| Slice 0033: Varroa Photo Analysis Evidence And Adapter Readiness | Technical | Make the Varroa Detector runtime path safely replaceable, QA-runnable, and backed by persisted Photo Analysis and per-bee evidence records. | HiveSight can already count mites transiently with a deterministic stub; the next step is proving a real detector can plug into the same evidence path without silent fallback, ambiguous zeroes, or lost provenance. |
| One-Click Varroa Photo Analysis Workflow | Business / Technical | Add the beekeeper-facing Analyze Photo action on Varroa assessment inspections, running Bee Localisation, Bee Orientation, and Varroa Detection as one workflow and showing summary-first evidence review. | Slice 0033 deliberately creates the safe adapter and evidence foundation first; the working inspection page should immediately follow so the real user flow is not left as hidden technical debt. |
