...
| Legacy Core API Gherkin Catalogue Cleanup | Technical / Quality | Complete the remaining migration decision pass for legacy Core API Gherkin. Migrate current behaviours into `acceptance/features/<capability>/...` when touched or high-risk, and explicitly leave superseded/lower-level behaviours as historical evidence so technical debt is visible rather than hidden. Current next candidate when touched: Varroa Detector adapter seam. |
