...
HiveSight can now build, train, and benchmark an early crop-level Bee Localisation candidate, build and evaluate the first Bee Orientation baseline path, label bee-level Varroa Review Outcomes for human model-curation evidence, roll those reviewed outcomes into a current photo-visible Varroa evidence summary, and let a Workspace member analyse one Varroa Assessment Inspection Photo through a durable one-click product workflow. That workflow persists model-backed Inspection Photo Bee Evidence, reports bees with likely visible Varroa, supports reversible photo-level review, and shows a frozen confidence-policy outcome that separates development integration evidence from future product-candidate Advisor evidence. It does not yet trigger HiveSight Advisor from accepted evidence, provide corrective feedback/expert handling, or produce a statistically defensible photo-visible estimate.
...
| Slice 0033: Varroa Photo Analysis Evidence And Adapter Readiness | Technical | Make the Varroa Detector runtime path safely replaceable, QA-runnable, and backed by persisted Photo Analysis and per-bee evidence records. | Delivered as the safe adapter and evidence foundation for the product-facing analysis workflow. |
| Slice 0034: One-Click Varroa Photo Analysis Workflow | Business / Technical | Add the beekeeper-facing Analyze Photo action on Varroa assessment inspections, running Bee Localisation, Bee Orientation, and Varroa Detection as one workflow and showing summary-first evidence review. | Delivered with a durable running state, deterministic replaceable product adapter, per-photo results, batch convenience action, and reversible review. |
| Slice 0035: Product Photo Analysis Confidence Policy | Business / Technical | Decide how low-confidence Bee Localisation, Bee Orientation, and Varroa Detection outputs affect product evidence, caveats, review, and any future estimate. | Delivered with frozen policy status, explicit Advisor evidence eligibility, visible caveats, API/browser coverage, and live Postgres persistence verification. |
| Slice 0036: Hive Frame Slot And Inspection Photo Side Context | Business / Technical | Add brood-slot frame-position context, per-inspection slot observations, continuity evidence, and side-labelled Inspection Photos for Varroa Assessment inspections. | Promoted after domain-model grilling showed that brood slot position and skipped/inactive evidence are needed before longitudinal frame evidence or treatment comparison can be trusted. |
| Product Photo Analysis Per-Bee Confidence Breakdown | Business / Technical | Explain per-bee and photo-level confidence across bee presence/completeness, orientation usability, and Varroa positive/negative evidence. | Parked from Slice 0035 so the run-level policy can stay thin while preserving the need for trustworthy per-bee evidence explanation before real adapter promotion or Advisor use. |
| Product Photo Analysis Corrective Feedback And Expert Review | Business / Governance | Let a beekeeper flag incorrect bee/orientation/mite evidence and submit it into a managed expert-review response workflow without silently turning product evidence into training data. | This is intentionally separate from Slice 0034's photo-level review so corrective feedback receives its own provenance, governance, and response design. |
| Photo Analysis Reanalysis And Retention | Business / Technical | Reanalyse a historical Inspection Photo using a newer model as a new immutable evidence run, and define logical deletion/retention of source photos and derivative evidence. | Reanalysis must not rewrite prior evidence; deletion and retention need an explicit policy before user-facing controls exist. |
...
| Candidate Slice | Type | Outcome |
| --- | --- | --- |
| Capability-Based Acceptance Catalogue Migration | Technical / Quality | Started after Slice 0030: keep active behaviours in `acceptance/features/<capability>/...`, tagged by seam. Varroa review is dual API/Web; Photo-visible Varroa evidence summary and Advisor treatment recommendation intake are API-bound catalogue coverage. Continue capability-by-capability, prioritising current drift risk over historical completeness. |
| Legacy Core API Gherkin Catalogue Cleanup | Technical / Quality | Complete the remaining migration decision pass for legacy Core API Gherkin. Migrate current behaviours into `acceptance/features/<capability>/...` when touched or high-risk, and explicitly leave superseded/lower-level behaviours as historical evidence so technical debt is visible rather than hidden. Slice 0033 migrated the touched Varroa detector/photo-analysis behaviour; remaining candidates should be selected by drift risk. |
| Slice 0031: Acceptance Catalogue Inventory And Labels | Technical / Quality | Delivered initial inventory and labels for canonical catalogue features, legacy API BDD, browser BDD, browser acceptance specs, and harness/reporting guards. Slice 0032 migrated the Photo-visible Varroa evidence summary; remaining cleanup is tracked separately. |
